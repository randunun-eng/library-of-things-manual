...
```mermaid
graph TD
    A[Start] --> B[Obtain Reference Number]
    B --> C[Request & Verify Identity]
    C --> D[Select Subscription & Pay]
    D --> E[Accept Agreement]
    E --> F{Active Member}
    
    F --> G[Book Item via Digital Platform]
    G --> H[Physical Borrowing]
    H --> I[Use & Learn]
    I --> J[Return Item on Time]
    J --> K{Condition Check}
    
    K -- Good --> L[Earn Points & XP]
    K -- Damaged/Late --> M[Deduct Points]
    
    L --> N[Level Up & Unlock New Items]
    M --> F
    N --> F
```
